...
Alpha 历史与截面均包含以下长期模型利差暴露，截面还提供同组分位：
...
截面返回 `gamma_policy_percentile`、`gamma_secondary_percentile`、`gamma_cpnote_percentile`、
...
以下解释由久期与 Alpha 分析共用。因子是代表性曲线的风险代理，
描述净值对相应风险的敏感性，不直接对应持仓比例。具体增减持或仓位比例由持仓数据支持。
