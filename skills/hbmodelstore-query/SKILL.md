...
description: 通过 hbmodelstore 统一公网 API 发现和查询公开模型数据，生成单基金久期、期限暴露、利差暴露图或三图联动页面，并在用户询问模型构建逻辑、方法选择、版本演进、解释细节、验证结果或消融实验时自动读取网页端最新的机器可读模型文档后作答；用于列出可用模型，查询和可视化纯债基金修正久期、因子暴露及已发布的 Alpha 信号。无需 API Key，不直接连接 PostgreSQL，不用于模型接入、数据库管理、生产任务或服务器运维。
---
